...
const PharmacyContent = () => {
  ...
  const stats = [
    { title: "Medication SKU", value: "1,240", subtitle: "Active Registry", icon: "cube", iconColor: "#3b82f6", bgColor: "#eff6ff" },
    { title: "Stock Value", value: "₹42K", subtitle: "Operational Flow", icon: "cash", iconColor: "#10b981", bgColor: "#f0fdf4" },
    { title: "Low Stock", value: "14", subtitle: "Alert Required", icon: "alert-circle", iconColor: "#f59e0b", bgColor: "#fffbeb" },
    { title: "Outflow", value: "842", subtitle: "Daily Registry", icon: "repeat", iconColor: "#e11d48", bgColor: "#fff1f2" },
  ];

  const renderBarChart = () => (
    <DashSection title="Dispensing Analytics" subtitle="Medical SKU Outflow" icon="stats-chart" iconBg="#8b5cf6">
      <View className="h-32 justify-end flex-row px-2">
          <Svg height={100} width={width - 80}>
             {[40, 60, 50, 80, 45, 70, 90].map((v, i) => (
               <Rect key={i} x={i * ((width-130)/6)} y={100-(v)} width={14} height={v} fill="#0052CC" rx={4} />
             ))}
          </Svg>
      </View>
    </DashSection>
  ...
  return (
    <View className="flex-1 bg-slate-50">
      ...
      <View className="px-6 py-6 bg-white border-b border-slate-100 shadow-sm">
        <View className="flex-row items-center justify-between">
          <View className="flex-row items-center">
            ...
            <View>
              <Text className="text-2xl font-black text-slate-900 tracking-tighter">Pharmacy</Text>
              <Text className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Institutional Store Core</Text>
            </View>
          </View>
        ...
        <View className="flex-row mt-6 gap-3">
          <TouchableOpacity className="flex-1 flex-row items-center justify-center bg-blue-600 py-4 rounded-2xl">
            ...
            <Text className="ml-2 text-xs font-black text-white">Add Stock</Text>
          </TouchableOpacity>
          <TouchableOpacity className="flex-1 flex-row items-center justify-center bg-slate-50 border border-slate-100 py-4 rounded-2xl">
            <Ionicons name="barcode" size={18} color="#64748b" />
            <Text className="ml-2 text-xs font-black text-slate-700">Scan SKU</Text>
          </TouchableOpacity>
        </View>
      </View>
      ...
      <ScrollView className="flex-1" contentContainerStyle={{ padding: 20, paddingBottom: 100 }}>
        ...
        {renderBarChart()}
        <DashSection title="Supply Registry" subtitle="Active Inventory" icon="cube" iconBg="#10b981">
               ...
               <View key={i} className="px-5 py-5 flex-row items-center justify-between border-b border-slate-100">
                 <View className="flex-row items-center flex-1">
                   <View className="w-10 h-10 items-center justify-center bg-white rounded-xl shadow-sm mr-4 border border-slate-100">
                      <Ionicons name="bandage-outline" size={18} color="#0052CC" />
                   </View>
                   <View>
                     <Text className="text-sm font-black text-slate-800">Paracetamol {650+i}mg</Text>
                     <Text className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter">SKU: {10200+i} • Unit Stock: {400+i*10}</Text>
                   </View>
                 </View>
                 <Text className="text-[8px] font-black text-emerald-600 uppercase bg-emerald-50 px-2 py-1 rounded-md">High Stock</Text>
               </View>
             ))}
           </View>
        </DashSection>
...
const PharmacyScreen = () => (<AdminLayout><PharmacyContent /></AdminLayout>);
